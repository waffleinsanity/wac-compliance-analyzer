...
from app.services.wac_scope import (
    allegation_cites_only_selected,
...
def _local_investigate(
    ...
    notes_lines = [
        "Local investigator analysis (LLM unavailable or disabled).",
        "Scope locked to selected WACs only.",
        "",
        "Known:",
...
def _parse_llm_result(
    ...
    *,
    ...
    return InvestigatorResult(
        ...
        next_steps=[_clean(str(x)) for x in (data.get("next_steps") or [])][:10],
        recommended_subsections=recommended,
        codes=codes_out,
...
def run_investigator(
    ...
    *,
    ...
    """Scoped local investigator by default; optional LLM enrichment when use_llm=True."""
    selected: list[dict[str, Any]] = []
    seen: set[str] = set()
    for node in selected_nodes:
        ...
        if getattr(node, "level", "code") != "code":
            # Prefer code-level text; skip pure subsection nodes here
            continue
        seen.add(code)
        selected.append(
            {
                ...
                # Truncate — local/LLM ranking uses subsections from the store, not this blob
                "text": _clean(getattr(node, "text", "") or "")[:4000],
            }
        )

    if not selected:
        # Promote from any nodes
        for node in selected_nodes:
            code = getattr(node, "code", "") or ""
    ...
    if not want_llm:
        result = _local_investigate(complaint, selected)
    ...
    messages = [
        ...
        {"role": "user", "content": _build_user_prompt(complaint, selected)},
    ]
    try:
        raw = _chat_completion(messages)
        data = _extract_json(raw)
        return _parse_llm_result(data, selected, model=settings.llm_model, complaint=complaint)
