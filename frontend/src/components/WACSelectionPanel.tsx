      ...
      <div className="space-y-3 border-b p-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search approved WAC codes..."
            value={searchQuery}
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="space-y-2 p-4">
          {groupedCodes.map(({ key, label, codes }) => {
            ...
            return (
              <div key={key}>
                <div className="flex cursor-pointer items-center gap-2 rounded-xl border border-border/70 bg-card/80 p-3 shadow-sm hover:bg-muted/50">
                  <button type="button" className="flex flex-1 items-center gap-2 text-left" onClick={() => toggleExpansion(key)}>
                    {open ? <ChevronDown className="h-4 w-4 text-tide-600" /> : <ChevronRight className="h-4 w-4 text-ink-400" />}
                    <input
                      type="checkbox"
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {selectedInSection}/{ids.length} approved for case
                      </div>
                    </div>
